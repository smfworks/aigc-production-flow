...
FastAPI spine. When `STUDIO_IMAGINE_URL` is set and Imagine health reports `imagine_configured`, Quick create is the Create screen: `POST /api/quick/plan` (no spend), `POST /api/quick/run` with `confirm: true` (jobs permission, 409 otherwise), `GET /api/quick/{run_id}` until `episode.mp4` is in the media store. `called_comfy` stays false. `produced_mp4` is true only after that download. Gates and generate-ok are unchanged. Token `STUDIO_IMAGINE_TOKEN` (default `local-dev-token`). Poll `STUDIO_IMAGINE_POLL_SECONDS` (default 5). Budget key `grok-imagine` in `STUDIO_COST_RATES`.
