...
**Start here:** open Studio. Create opens on **Quick create**: one story, a length, an aspect, a local plan, then a confirmed run. That run saves the episode on the local desk. It does not render a film and does not call a cloud image or video API. Gates, sign-off, and generate-ok are unchanged. `called_comfy` stays false. `produced_mp4` stays false. **Full wizard** is still there. **Send to Hermes** writes a brief. Download agent zip is the fallback.
...
Phase 16 adds the CLIP_BRIDGE Comfy API stubs under `studio/fixtures/clip_bridge/workflows/`. `docs/CLIP_BRIDGE_COMFY.md` locks the widgets: Qwen steps 25, cfg 1, euler, simple; H3 API `MinimaxHailuo03FirstLastFrameNode`, duration 10, resolution 2K. Inject fills those paths and does not queue. Prefer the API H3 graph for production 2K / 10s. The native graph is 1344×768, length 243. Extract and stitch stay on ffmpeg (`studio/scripts/clip_bridge_conform.sh`, CLIP_BRIDGE §12).

Phase 15 sits on that preview. `docs/CLIP_BRIDGE.md` is the long-form continuity dialect. Generate for a bridge clip shows the assembled H3 prompt and the Qwen start and end strings, filled from the story lock and the clip. Locked `start_state` and `end_state` strings are copied, not paraphrased. A failed continuity check leaves Generate off. ffmpeg conform extracts a last frame and trims a held tail only when the clip file is on disk. A missing file stays awaiting conform. No MP4 is invented. `called_comfy` stays false until a live lane accepts a prompt. This is not a second Hermes app and not CapCut.
...
Job rows store `estimated_cost_units` at enqueue and `actual_cost_units` on success (0 on fail/cancel). Units come from `STUDIO_COST_RATES` (default `stub:0.1,webhook:1,cli:1,comfy-h3:2,comfy-qwen:0.5`). `STUDIO_COST_CURRENCY` defaults to `credits`. Optional `STUDIO_COST_USD_PER_UNIT` is an estimate only. Quick create does not spend units. It is not a cloud invoice.
