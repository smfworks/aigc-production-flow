...
Open Studio. Create starts on Quick create: one story prompt, 15/30/60 seconds, 9:16 / 16:9 / 1:1, an editable shot plan, then **Run**. Run saves the episode on the local desk. It does not render a film. `called_comfy` stays false. `produced_mp4` stays false. Gates, sign-off, and generate-ok do not move. **Full wizard** is the link to the longer door. Studio does not call a cloud image or video API.
